...
In order to make this work we will have to convert our qwi dataframe to a data.table object within our *dcast.data.table()* call. Note that *dcast.data.table()* uses a formula type interface to name the variables we want to transform. In this case we tell data.table to take the avg_emp and avg_payroll variables and spread them out by metro area and year.
...
qwi_tot_wide <- qwi_tot_wide %>% 
  mutate(emp_growth = (avg_emp_2016 - avg_emp_2007)/avg_emp_2007,
         payroll_growth = (avg_payroll_2016 - avg_payroll_2007)/avg_payroll_2007)
```
...
kable(qwi_annual_wide %>% select(1:3, 8:9, 12:13) %>% head(), "latex", caption = "MSA Location Quotients for 2007 and 2016", booktabs = TRUE) %>% 
  kable_styling(latex_options = c("striped", "scale_down"))
